...
function NavItem({ item }: { item: MenuItem }) {
  ...
  // A flat item (no grandchildren) gets a small anchored dropdown; only nested
  // menus warrant the full-width column panel below.
  if (item.items.every((child) => child.items.length === 0)) {
    return (
      <li className="group relative flex items-center h-16">
        <MenuLink url={item.url} className={TRIGGER_CLASS}>
          {item.title}
        ...
        >
          <ul className="min-w-44 rounded-md border border-border bg-background py-1.5 shadow-md">
            {item.items.map((leaf) => (
              <li key={leaf.id}>
                <MenuLink
                  url={leaf.url}
                  className="block px-3.5 py-1.5 text-sm transition-colors hover:bg-muted"
                >
                  {leaf.title}
                </MenuLink>
              </li>
