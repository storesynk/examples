...
// Tagged "cart" so every mutation invalidates it (invalidateCartCache here,
// invalidateCartTag in the @storesynk/next actions — same tag). Keyed by cart
// id; the TTL is a safety net for changes Shopify makes without a webhook.
export async function getCachedCartById(cartId: string): Promise<Cart | undefined> {
  "use cache";
  cacheTag("cart");
  cacheLife("hours");
  return fetchCart(cartId);
